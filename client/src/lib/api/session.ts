...
export async function updateSession(user: User, session: Session) {
    if (!user) {
        return {
            result: null,
            status_code: 401,
...
export async function getAllSessions(user: User) {
...
export async function doesSessionExists(user: User, routine_id: string): Promise<DbResponse<Session>> {
    if (!user) {
        return {
            result: null,
            status_code: 401,
    ...
    try {
        ...
        const { token } = user;
        const url = generateUrl(RequestTarget.DoesSessionExist, routine_id);
        const res = await fetch(url, postReqeust(token, ""));
